...
class InstagramRecommenderReal:
    def __init__(self):
        self.db = mysql.connector.connect(
            ...
            password="2929", database="instagram_ml_real"
        )
    ...
    def load_real_data(self):
        self.cursor.execute("""
            SELECT u.username, p.post_name, p.image_path, p.caption,
        ...
        posts_dict = {row[1]: {"image_path": row[2] or f"images/{row[1]}.jpg",
                               "caption": row[3] or f"Post {row[1]}"} 
                     for row in data}
        posts_df = pd.DataFrame(posts_dict).T

        for username, post_name, _, _, _ in data:
    ...
    def load_image(self, post_name, size=(150, 150)):
        ...
        image_path = self.posts_df.loc[post_name]["image_path"]
        fallback_paths = [image_path, f"images/{post_name}.jpg", "images/default.jpg"]
    ...
    def create_default_image(self, size, post_name):
        ...
        ImageDraw.Draw(img).text((10, 10), post_name[:10], fill="black")
        photo = ImageTk.PhotoImage(img)
        self.image_cache[post_name] = photo
        return photo
    ...
    def get_real_recommendations(self, target_user, n=5):
        ...
        top_similar = sorted([(u, s) for u, s in sim_scores.items() if u != target_user],
                           key=lambda x: x[1], reverse=True)[:3]

        target_likes = set(self.user_post_matrix.loc[target_user][
            self.user_post_matrix.loc[target_user] > 0].index)
        recommendations = {}
        
        for sim_user, score in top_similar:
            if sim_user in self.user_post_matrix.index:
                sim_likes = set(self.user_post_matrix.loc[sim_user][
                    self.user_post_matrix.loc[sim_user] > 0].index)
                for post in sim_likes - target_likes:
                    if post in self.posts_df.index:
                        recommendations[post] = max(recommendations.get(post, 0), score)

        sorted_posts = sorted(recommendations.items(), key=lambda x: x[1], reverse=True)[:n]
        return sorted_posts, [u for u, _ in top_similar]
...
class InstagramRealApp:
    ...
    def setup_ui(self):
        ...
        tk.Label(header, text="📱 Instagram ML Recommender", 
                font=("Arial", 24, "bold"), bg="#1877f2", fg="white").pack(pady=10)
        tk.Label(header, text="Recommandations personnalisées", 
                font=("Arial", 11), bg="#1877f2", fg="white").pack()
        ...
        tk.Label(left_frame, text="👤 Choisir un utilisateur", 
                font=("Arial", 14, "bold"), bg="#f0f2f5").pack(pady=(10, 10))
    ...
    def get_real_recs(self):
        if not self.recommender.users:
            messagebox.showwarning("Aucune donnée", "Aucun utilisateur trouvé.")
            return
    ...
    def display_real_posts(self, target_user, top_posts, similar_users):
        ...
        if top_posts:
            ...
            stats_text += f"\nPosts : {len(top_posts)} | Score moyen : {np.mean(scores):.2f}"
        else:
            stats_text += "\nAucune recommandation trouvée."
        self.stats_label.config(text=stats_text)

        if not top_posts:
            tk.Label(self.scrollable_frame, text="😴 Aucune recommandation", 
                    font=("Arial", 14), bg="white", fg="#999999").pack(pady=40)
            return
        ...
        for i, (post_name, score) in enumerate(top_posts, start=1):
            ...
            tk.Label(info, text=f"{i}. {post_name}", font=("Arial", 12, "bold"),
                    bg="white", fg="#222222").pack(anchor="w")
            
            caption = self.recommender.posts_df.loc[post_name, "caption"] \
                if post_name in self.recommender.posts_df.index else "Caption indisponible"
            if len(caption) > 140:
                caption = caption[:140] + "..."
            tk.Label(info, text=caption, font=("Arial", 10), bg="white", fg="#555555",
                    wraplength=500, justify="left").pack(anchor="w", pady=(5, 5))

            tk.Label(info, text=f"🤖 Score ML : {score:.3f}", font=("Arial", 10, "bold"),
                    bg="white", fg="#1877f2").pack(anchor="e")
